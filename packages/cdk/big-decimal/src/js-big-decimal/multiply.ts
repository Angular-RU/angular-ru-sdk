/* eslint-disable */
export function multiply(number1: any, number2: any) {
    ...
    for (let i = 0; i < 2 * number1.length; i++) {
        ...
        for (let j = number2.length - 1; j >= currentPosition && j >= 0; j--) {
            if ((positionVector[j] ?? 0) > -1 && (positionVector[j] ?? 0) < number1.length) {
 ...
 * Add decimal point
 */
function adjustDecimal(number: any, decimal: any) {
