...
describe('[TEST]: Eslint', (): void => {
    ...
    it('check failed files', (): void => {
        ...
        expect(bad.includes(`Don't use \`Function\` as a type. The \`Function\` type accepts any function-like value.`)).toEqual(true);
        expect(bad.includes(`Class name \`little\` must match one of the following formats: PascalCase`)).toEqual(true);
        expect(bad.includes(`Class name \`Little_Bar\` must match one of the following formats: PascalCase`)).toEqual(true);
        expect(bad.includes(`Interface name \`myType\` must match one of the following formats: PascalCase`)).toEqual(true);
        expect(bad.includes(`Type Alias name \`alist\` must match one of the following formats: PascalCase`)).toEqual(true);
