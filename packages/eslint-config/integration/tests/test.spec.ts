...
describe('[TEST]: Angular-RU eslint recommendations', (): void => {
    ...
    it('check file pattern in directories', () => {
        ...
        expect(filePattern.includes('2 problems (2 errors, 0 warnings)')).toEqual(true);
        expect(filePattern.includes("Files doesn't must end with *.model.ts  @angular-ru/no-model-suffix")).toEqual(
            true
        );
        expect(filePattern.includes("Files doesn't must end with *.enum.ts  @angular-ru/no-enum-suffix")).toEqual(true);
    });
});
