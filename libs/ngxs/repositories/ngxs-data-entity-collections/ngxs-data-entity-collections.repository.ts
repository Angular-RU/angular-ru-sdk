    ...
    implements EntityRepository<V, K, C>
{
    ...
    public get ids$(): Observable<K[]> {
        return this.state$.pipe(
            map((value: EntityCollections<V, K, C>): K[] => value.ids),
        );
    }
    ...
    public get entities$(): Observable<EntityDictionary<K, V>> {
        return this.state$.pipe(
            map(
                (value: EntityCollections<V, K, C>): EntityDictionary<K, V> =>
                    value.entities,
            ),
        );
    }
    ...
    public get entitiesArray$(): Observable<V[]> {
        return this.state$.pipe(
            map((value: EntityCollections<V, K, C>): V[] =>
                value.ids.map((id: K): V => value.entities[id]),
            ),
        );
    }

    protected get ctx(): EntityContext<V, K, C> {
        return ensureDataStateContext<
            EntityCollections<V, K, C>,
            StateContext<EntityCollections<V, K, C>>
    ...
    @DataAction()
    public patchState(
        @Payload('patchValue') value: Partial<EntityCollections<V, K, C>>,
    ): void {
    ...
    protected setEntitiesAll(entities: V[]): void {
        ...
        this.setEntitiesState({
            ...state,
            ids,
            entities: dictionary as EntityDictionary<K, V>,
        });
    }
    ...
    protected updateEntitiesMany(updates: EntityUpdate<V, K>[]): void {
        ...
        this.setEntitiesState({
            ...state,
            ids: state.ids.map((id: K): K => keys[id] ?? id),
            entities,
        });
    }
    ...
    protected removeEntitiesMany(ids: K[]): void {
        ...
        this.setEntitiesState({
            ...state,
            ids: state.ids.filter((id: K): boolean => id in keys),
            entities,
        });
    }
    ...
    protected sortKeysByComparator(
        originalIds: K[],
        ...
        if (typeof comparator === 'function') {
            return ids.sort((a: K, b: K): number =>
                comparator(entities[a] as V, entities[b] as V),
            );
        }
    ...
    private sortByComparatorOptions(
        ids: K[],
        comparator: EntitySortBy<V>,
        entities: EntityDictionary<K, V>,
    ): K[] {
        switch (comparator?.sortByOrder) {
            ...
            default:
                if (isDevMode()) {
                    console.warn(
                        `Invalid --> { sortByOrder: "${comparator?.sortByOrder}" } not supported!`,
                    );
                }
    ...
    private generateKeyMap(state: EntityCollections<V, K, C>): KeysDictionary<K> {
        return state.ids.reduce(
            (keyDictionary: KeysDictionary<K>, id: K): KeysDictionary<K> => {
                keyDictionary[id] = id;
