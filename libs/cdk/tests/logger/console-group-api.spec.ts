...
describe('[TEST]: Check work in groups', () => {
    describe('case #1', () => {
        ...
        it(`pipe group-collapsed`, () => {
            ...
            expect(fakeConsole.stack()).toEqual(
                fakeConsole.createStack(
                    { [TestLoggerGroupType.GROUP_COLLAPSED_OPEN]: [`group collapsed name`] },
                    { [TestLoggerLineType.TRACE_OR_DEBUG]: [traceIsWork] },
                    { [TestLoggerLineType.DEBUG]: [debugIsWork] },
                    { [TestLoggerLineType.INFO]: [infoIsWork] },
                    { [TestLoggerLineType.WARN]: [warnIsWork] },
        ...
        it(`level pretty groups`, () => {
            ...
            logger.debug.group('B opened', ({ debug }: LoggerService) => debug(debugGroupIsWork));
            logger.info.group('C opened', ({ info }: LoggerService) => info(infoGroupIsWork));
            logger.warn.group('D opened', ({ warn }: LoggerService) => warn(warnGroupIsWork));
            logger.error.group('E opened', ({ error }: LoggerService) => error(errorGroupIsWork));

            logger.level = LoggerLevel.INFO;

            logger.trace.groupCollapsed('A collapsed', ({ trace }: LoggerService) => trace(traceGroupIsWork));
            logger.debug.groupCollapsed('B collapsed', ({ debug }: LoggerService) => debug(debugGroupIsWork));
            logger.info.groupCollapsed('C collapsed', ({ info }: LoggerService) => info(infoGroupIsWork));
            logger.warn.groupCollapsed('D collapsed', ({ warn }: LoggerService) => warn(warnGroupIsWork));
            logger.error.groupCollapsed('E collapsed', ({ error }: LoggerService) => error(errorGroupIsWork));
