...
export function roundOff(input: number | string, n: number = 0, mode = RoundingModes.HALF_EVEN): any {
    ...
    if (n < 0) {
        n = -n;
        if ((partInt?.length ?? 0) <= n) return '0';
        else {
            let prefix = partInt?.substr(0, (partInt?.length ?? 0) - n);
            input = prefix + '.' + partInt?.substr((partInt?.length ?? 0) - n) + partDec;
...
function greaterThanFive(part: any, pre: any, neg: boolean, mode: RoundingModes) {
    if (!part || part === new Array(part.length + 1).join('0')) return false;

    // #region UP, DOWN, CEILING, FLOOR
    if (
        mode === RoundingModes.DOWN ||
        (!neg && mode === RoundingModes.FLOOR) ||
...
function increment(part: any, c: number = 0) {
    if (!c) c = 1;
