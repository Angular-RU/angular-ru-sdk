...
export function dateMaxIntervalValidator(
    ...
    return dateIntervalValidator(
        intervalLimit,
        descriptor,
        DateIntervalValidatorType.LIMIT_MAX_INTERVAL,
    );
}
...
export function dateMinIntervalValidator(
    ...
    return dateIntervalValidator(
        intervalLimit,
        descriptor,
        DateIntervalValidatorType.LIMIT_MIN_INTERVAL,
    );
}

export function orderedIntervalValidator({
    dateFromKey,
    dateToKey,
}: DateIntervalValidatorDescriptor): ValidatorFn {
    return (formGroup: AbstractControl): ValidationErrors | null => {
        assertFormGroup(formGroup, DATE_INTERVAL_VALIDATOR);
        const validatorType: DateIntervalValidatorType =
            DateIntervalValidatorType.ORDERED_INTERVAL;
        ...
        return (from ?? 0) > (to ?? 0)
            ? ({[validatorType]: true} as ValidationErrors)
            : null;
    };
}
...
function dateIntervalValidator(
    ...
    return (formGroup: AbstractControl): ValidationErrors | null => {
        assertFormGroup(formGroup, DATE_INTERVAL_VALIDATOR);
        const formGroupValue: PlainObject = formGroup.getRawValue();
        const interval: Nullable<number> = getDateInterval(
            formGroupValue[dateFromKey],
            formGroupValue[dateToKey],
        );
