        ...
        OnDestroy
{
    ...
    public ngOnChanges(changes: SimpleChanges): void {
        ...
        this.changesTimerId = window.setTimeout(
    ...
    protected calculateViewPortByRange({
        start,
        end,
        bufferOffset,
        force,
    ...
    private checkCorrectInitialSchema(changes: SimpleChanges = {}): void {
        if (TableSimpleChanges.SCHEMA_COLUMNS in changes) {
            const schemaChange: Nullable<SimpleChange> =
    ...
    private checkIfKeysAreDifferent(): boolean {
        return (
            this.sourceExists &&
            (this.getKeys().length !== this.renderedKeys.length ||
                !this.renderedKeys.every(include(this.getKeys())))
        );
    }
    ...
    private updateViewport(): void {
        ...
        this.frameCalculateViewportId = window.requestAnimationFrame((): void =>
    ...
    private getOffsetVisibleEndIndex(): number {
        return (
            Math.floor(
                (this.scrollOffsetTop + this.viewportHeight) / this.clientRowHeight,
            ) - 1
        );
    }
    ...
    private syncDrawColumns(columnList: string[]): void {
        for (const [index, key] of columnList.entries()) {
            const schema: Nullable<ColumnsSchema> = this.getCompiledColumnSchema(
                key,
                index,
            );
    ...
    private getCompiledColumnSchema(key: string, index: number): Nullable<ColumnsSchema> {
        const customColumn: Partial<ColumnsSchema> =
        ...
        const defaultColumn: Nullable<ColumnsSchema> =
    ...
    private processedColumnList(schema: ColumnsSchema, key: Nullable<string>): void {
        const hasSchema: boolean = checkValueIsFilled(
            (this.templateParser.schema ?? schema) as any,
        );

        if (hasSchema) {
            const compiledSchema: Nullable<ColumnsSchema> =
                this.templateParser.compiledTemplates[key as string];
